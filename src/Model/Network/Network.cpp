...
std::vector<std::vector<std::string>> NetWork::read_file(std::string &fname) {
...
void NetWork::set_input(const std::vector<double> &values) {
  for (int i = 0; i < size[0]; i++) {
    neurons_val[0][i] = values[i];
  }
}

int NetWork::forward_feed() {
  for (int k = 1; k < L; ++k) {
...
int NetWork::search_max_index(std::vector<double> &value) {
  if (!value.empty()) {
...
void NetWork::print_values(int &L) {
  for (int j = 0; j < size[L]; j++) {
    std::cout << j << " " << neurons_val[L][j] << std::endl;
  }
}

void NetWork::back_propogation(double &expect) {
  for (int i = 0; i < size[L - 1]; i++) {
    if (i != int(expect))  //-1*
      ...
      delta[L - 1][i] =
          -1 * (1.0 - neurons_val[L - 1][i]) *
          actFunc.useDer(neurons_val[L - 1][i]);  // value*(1-value)
  }
  for (int k = L - 2; k > 0; k--) {
    Matrix::multi_t(weights[k], delta[k + 1], size[k + 1], delta[k]);
    for (int j = 0; j < size[k]; j++)
      delta[k][j] *= actFunc.useDer(neurons_val[k][j]);
  }
}

void NetWork::weights_updater(const double &lr) {  // lr 0.08 or 0.3
  for (int i = 0; i < L - 1; ++i) {
    for (int j = 0; j < size[i + 1]; ++j) {
      for (int k = 0; k < size[i]; ++k) {
        weights[i](j, k) -=
            (1 - step) * neurons_val[i][k] * delta[i + 1][j] * lr;
      }
    }
  }
}

void NetWork::save_weights(std::string &file) {
  std::ofstream fout;
...
void NetWork::read_weights(std::string &file) {
  std::ifstream fin;
...
void NetWork::error_test(int &y_test) {
...
std::pair<double, double> NetWork::training_(std::string &train_file_,
                                             const double &lr,
                                             const int &epoch) {
  std::pair<double, double> res;
  std::vector<std::vector<std::string>> content_train;
  std::vector<double> train_ = {};
  content_train = s21::NetWork::read_file(train_file_);
...
std::pair<double, double> NetWork::testing_(std::string &test_file_) {
  std::pair<double, double> res;
  std::vector<std::vector<std::string>> content_test;
  std::vector<double> test_ = {};
  content_test = s21::NetWork::read_file(test_file_);
  std::chrono::duration<double> time{};
  ...
  res.first = accur_ * 100;
  res.second = time.count() / 60.;
  return res;
}

std::pair<double, double> NetWork::start_training(std::string &train_file,
  ...
  s21::NetWork netw;
  netw.init(L);
  std::pair<double, double> res = netw.training_(train_file, lr, epoch);
  netw.save_weights(save_file);
  return res;
}

std::pair<double, double> NetWork::start_testing(std::string &test_file,
                                                 std::string &save_file,
                                                 const int &L) {
  s21::NetWork netw;
  netw.init(L);
  std::string file = "/Users/luigiket/CPP7_MLP-1/src/Model/Weights/weigth_3.txt";
  netw.read_weights(file);
  std::pair<double, double> res = netw.testing_(test_file);
  netw.save_weights(save_file);
  return res;
}

int NetWork::ready_weights(const std::vector<double> &input_) {
  s21::NetWork netw;
  int L = 4;
  netw.init(L);
  std::string path_ =
      "/Users/luigiket/CPP7_MLP-1/src/Model/Weights/weigth_3.txt";
  netw.set_input(input_);
  netw.read_weights(path_);
  return netw.forward_feed();
}

std::pair<std::vector<std::vector<std::string>>,
          std::vector<std::vector<std::string>>>
NetWork::split_data(const std::vector<std::vector<std::string>> &data,
...
std::pair<double, double> NetWork::cross_validation(const int &L,
                                                    std::string &file,
                                                    const int &k,
                                                    const int &epoch,
                                                    const double &lr) {
  ...
  s21::NetWork netw;
  netw.init(L);
  vec = s21::NetWork::read_file(file);
  double fraction = 0.8;
